...
final class StringActionTests: XCTestCase {
    fileprivate func action(_ string: String) throws -> StringAction {
    ...
    func testFromURLComponents_WithoutPayload() throws {
        ...
        XCTAssertThrowsError(try action("protocol://host/path1"))
        XCTAssertThrowsError(try action("protocol://host"))
    ...
    func testFromURLComponents_WithPayload() throws {
        ...
        XCTAssertThrowsError(try action("protocol://host/path1?irrelevant=yes"))
        XCTAssertThrowsError(try action("protocol://host?irrelevant=yes"))
